...
printf("Ingrese un estado valido(1- Activo/0- Retirado,reintente...\n");
...
do{
    ...
    case 5:
        ...
        }while(verificarEstadoAstro(astro->estado)==0);
        break;
    case 6:
        opsw == -1;
        break;
...
int validosAstro = contarRegistros(astronauta);
int i = 0;
FILE* archi;
stAstronauta aux[validosAstro];

archi = fopen(astronauta,"rb");
if(archi !=NULL){

    while(fread(&aux[i],sizeof(stAstronauta),1,archi)>0){
        i++;
    }
...
printf("9-Buscar astronauta con mayor experiencia(Horas de vuelo):\n");
printf("10-Buscar astronauta con mayor experiencia(Misiones realizadas):\n");
...
case 1:
    ...
    dato = preguntarDato();
    dato = buscarIDastro(arregloAstro,dato,validos)?printf("El |ID: %i| corresponde a %s\n", arregloAstro[dato], arregloAstro[dato].apodo) : printf("|NO EXISTE EL ID|\n");
...
case 3:
    ...
    buscarNombreAstro(arregloAstro,datoString,validos) ?printf("Existen estos astronautas con el apellido: %s \n",datoString) : printf("No existen astronautas con ese apellido \n");
    break;
case 4:
    do{
    ...
    buscarNombreAstro(arregloAstro,datoString,validos) ?printf("Existen estos astronautas con el apodo: %s \n",datoString) : printf("No existen astronautas con ese apodo \n");
...
dato = 0;
do{
printf("Ingrese (1) para mostrar los astronautas activos \n");
fflush(stdin);
scanf("%i",&dato);
}while(validarEstado(dato)==0);/// si no anda probar con == 1
...
dato = 0;
do{
printf("Ingrese (0) para mostrar los astronautas activos \n");
fflush(stdin);
scanf("%i",&dato);
}while(validarEstado(dato)==1);/// si no anda probar con == 0
...
case 9:
      ...
      puts("--------------------ASTRONAUTA CON MAYOR EXPERIENCIA--------------------\n");
      mostrarUnAstronauta(aux);
    break;
case 10:
