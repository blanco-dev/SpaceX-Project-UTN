...
stMision crearUnaMision(char archivoMisiones[], char archivoNaves[], char archivoAstro[], int * IDmision)
{

    int cant=0, estadoNave=2;
    stMision aux;
    //int ultimaID = encontrarUltimaIDMision(archivoMisiones) + 1;

    *IDmision+=1;

    aux.estado = 1;   // autoincremental
    aux.IDMision = *IDmision  ; //ultimaID +1; // autoincremental
    ...
    cambiarEstadoDeNaveMision(archivoNaves,aux.IDNave,estadoNave); // aca cambio el estado de la nave a en mision ya que no quiero que se vuelva a elegir
...
void cargarArchivoMisiones(char archivoAstro[], char archivoNaves[], char archivoMision[])
{
    ...
    if(buffer != NULL)
    {
        aux =crearUnaMision(archivoMision,archivoNaves,archivoAstro,&ultimaID);
...
void modificarMisionLista(char archivoAstro[],char archivoMisiones[], char archivoNaves[])
{
    ...
    do
    {
        ...
        puts("O Ingrese -1 para salir al menu");
    ...
    while(flag == 0)
    {
        ...
        case 1:
            ...
            break;
        case 2:
            cancelarMision(&aux,archivoAstro,archivoNaves,maxTripu);
            flag= 1;
...
void agregarEstadisticasATripulantes(char archivoAstro[],int tripulantes[],int maxTripu, int horasDeVuelo)
{
    ...
    if(buffer != NULL)
    {
        ...
        for (int i=0; i < maxTripu; i++)
        {
            ...
            agregarUnaEstadisticaATripulante(pos,archivoAstro,horasDeVuelo);
...
void agregarUnaEstadisticaATripulante(int posicion,char archivoAstro[],int horasDeVuelo)
{
...
void modificarMisionEnVuelo(char archivoMisiones[],char archivoNaves[], char archivoAstros[])
{
    ...
    auxMision = retornarMisionXID(archivoMisiones,idMision);
